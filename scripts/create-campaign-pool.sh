#!/bin/bash
# create-campaign-pool.sh
# Replaces all 3 Lambdas (upload-campaign, create-jobs, start-campaign prep) for pool mode.
#
# Reads phones from data/phones.txt, picks images from S3 pool,
# creates job files (10 phones × 10 images = 100 submissions per job),
# uploads everything to S3, and prints EC2 export commands.
#
# Usage (run from repo root):
#   chmod +x scripts/create-campaign-pool.sh
#   ./scripts/create-campaign-pool.sh [campaign-name]
#
# Prerequisites:
...
echo ""
echo "=================================================="
echo "  CREATE CAMPAIGN (POOL MODE)"
...
echo "[3/5] Creating $TOTAL_JOBS jobs ($PHONES_PER_JOB phones × $IMAGES_PER_JOB images = 100 submissions each)..."
echo ""
...
EOF
  ...
  echo "  ✅ $JOB_ID | phones: ${#PHONE_GROUP[@]} | images: ${#IMG_GROUP[@]} | submissions: $SUBMISSIONS"
