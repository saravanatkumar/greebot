#!/bin/bash
# launch-campaign-spot-waves.sh
# Wave Launcher with SPOT INSTANCES - Launches batches in waves using AWS Spot Instances
# Each wave has multiple batches, each batch has ~42 jobs
# Logs all instances to shared state file for terminator script
#
# This version uses SPOT INSTANCES by default (save up to 90% on EC2 costs)
...
TOTAL_BATCHES_INPUT=8     # Total number of batches to process
...
# ========================================
export USE_SPOT=true                    # Enable spot instances
export SPOT_MAX_PRICE="${SPOT_MAX_PRICE:-0.05}"  # Max price per hour (default: $0.05)
...
echo "Spot max price: \$$SPOT_MAX_PRICE/hr"
echo ""
...
process_batch() {
  ...
  # Step 5: Launch EC2 instances (with SPOT enabled)
  echo "[Step 4/5] Launching EC2 SPOT instances..."
  ...
  echo "  Instances: $instance_count (SPOT)"
...
for wave_num in $(seq 1 $TOTAL_WAVES); do
  echo ""
  echo "========================================"
  echo "WAVE $wave_num OF $TOTAL_WAVES (SPOT INSTANCES)"
  echo "========================================"
  echo "Start time: $(date)"
  echo ""
...
echo ""
echo "========================================"
echo "  ALL WAVES LAUNCHED! (SPOT INSTANCES)"
echo "========================================"
echo "End time: $(date)"
echo "Total batches launched: $TOTAL_BATCHES_INPUT"
echo "Total waves: $TOTAL_WAVES"
echo "Instance type: SPOT (max \$$SPOT_MAX_PRICE/hr)"
echo ""
echo "💰 Estimated savings: 70-90% vs on-demand pricing"
echo ""
